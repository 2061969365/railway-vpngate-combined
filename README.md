...
全量真测：管理页按钮或 `POST /api/full_probe`（需 `Authorization: Bearer`）。每次刷新成功（开机/每小时/手动）都会自动在后台全量真测，按握手延迟从小到大 5 并发拨号，超时 20 秒。冷启动等首轮跑完才 pin（跑完定实测最低为主、次低为保底，chain 出 `[best, second, auto]`）；后续周期只刷新测速值、不重 pin、不换连接，pin 掉线/消失才换到当轮最优。手切过的节点不覆盖、平局不跳。状态条节点 title 显示主备。
